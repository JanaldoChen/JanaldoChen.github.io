...
我们假设 $$opt(a,j) > opt(b,j)$$ 其中 $$a<b$$

令 $$opt(a,j)=x_{a}$$ , $$opt(b,j)=x_{b}$$ .
...
$$
\begin{aligned} 
dp[x_b-1][j-1]+cost(x_b,b)&=dp[x_b-1][j-1]+cost(x_b,a)+cost(a+1,b)\\ &-(cost(x_b,a)\And cost(a+1,b)) \\
& <dp[x_a-1][j-1]+cost(x_a,a)+cost(a+1,b)\\ & -(cost(x_a,a)\And cost(a+1,b))\\
